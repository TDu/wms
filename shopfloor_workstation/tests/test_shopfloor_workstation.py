...
class ShopfloorWorkstationCase(CommonCase):
    def setUp(self):
        ...
        self.ws1.sudo().standard_printer_id = self.printer1
        self.profile1 = self.env.ref("shopfloor.shopfloor_profile_hb_truck_demo")
        self.ws1.sudo().shopfloor_profile_id = self.profile1
        self.ws2 = self.env.ref("shopfloor_workstation.ws_castor")
        self.ws2.sudo().standard_printer_id = self.printer2
                ...
                "records": [
                    {
                        ...
                        "name": "Pollux",
                        "barcode": "ws-1",
                        "warehouse": {"id": self.ws1.warehouse_id.id, "name": "YourCompany"},
                        "profile": {"id": self.ws1.shopfloor_profile_id.id, "name": "Highbay Truck"}
                    },
                ],
            },
        )
