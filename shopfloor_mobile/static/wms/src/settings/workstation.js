        ...
        on_scan: function(scanned) {
            this.odoo.call("setdefault", {barcode: scanned.text}).then(result => {
                ...
                if (result.error) {
                    ...
                    this.scan_message = {
                        "message_type": "error",
                        "body": "Request could not be processed, maybe the module shopfloor_workstation is not installed."
                        }
                    return;
                }
                ...
                if (this.scan_message.message_type === 'info') {
                    this.$root.trigger("profile:selected", this.scan_data.records[0].profile, true);
